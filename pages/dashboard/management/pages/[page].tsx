...
const Pages = (props: any) => {
    ...
    const cols = [
            ...
            render: (_: any, record: any) => {
                ...
                return editable ? (
                    <Select
                        style={{ width: "100%" }}
                        placeholder="Type"
                        onChange={(e: any) => { setTmpCurEditingSelect([...tmpCurEditingSelect,{ value: e, name: "type", index: data.findIndex((item: any) => record.key === item.key) }]) }}
        ...
        },
        {
            ...
            render: (_: any, record: any) => {
                const editable = isEditing(record);
                return editable ? <OptionsTag disabled={true} tagsName="options" setTmpCurEditingTags={props?.setTmpCurEditingTags} options={record["options"] ? typeof record["options"] === 'string' ? JSON.parse(record["options"]) : record["options"] : []} />
                : <OptionsTag disabled={true} tagsName="options" setTmpCurEditingTags={setTmpCurEditingTags} options={record["options"] ? typeof record["options"] === 'string' ? JSON.parse(record["options"]) : record["options"] : []} />;
                //return editable ? <OptionsTag tmpCurEditingTags={tmpCurEditingTags} tagsName="options" setTmpCurEditingTags={setTmpCurEditingTags} options={record.options ? record.options : []} /> : <OptionsTag disabled={true} tagsName="options" setTmpCurEditingTags={setTmpCurEditingTags} options={record.options ? record.options : []} />
            }
        },
        {
            title: 'Operation',
